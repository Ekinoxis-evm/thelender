...
2. **Score** — `POST /api/lendsignal/score`. The score is **100% the Chainlink Confidential AI Attester** over the borrower's uploaded documents, analyzed in a TEE (raw docs never leave the enclave or touch the DB). The pipeline is **map** (one inference per document, section prompts) → **reduce** (final decision); the combined score (0–1000) **equals** the AI score. There is **no** credit-bureau blend, no off-chain profile signal, and no synthetic/mock fallback. If `CHAINLINK_CONFIDENTIAL_AI_API_KEY` is missing or any inference fails / returns unparseable output, the route returns a **clear error** (never a fake score). Persisted to Supabase `credit_checks` (results + content hashes only). Eligibility: tiers `>=750` low · `400–749` medium · `<400` high; `eligible = score >= 400 AND tier != high` — recomputed from the stored score + tier against `MIN_ELIGIBLE_SCORE` (400), not a persisted flag. This `>=400` is the **identity-mint** threshold; the onchain **borrow** gate is stricter (the vault's `minScore`, currently 600).
...
5. **Borrow** — `KreditoVault.borrow(attestation, signature, amount, termMonths)`: the vault verifies the EIP-712 signature onchain (recovers signer == issuer), enforces score ≥ `minScore`, not-expired, `amount ≤ maxPrincipal`, then disburses an undercollateralized **installment** loan (equal-principal amortization). **Deployed on Sepolia.** The borrow gate (`minScore = 600`) is stricter than the identity-mint threshold (≥400) — a business can hold a credit identity yet need a higher score to draw a loan; the credit limit itself also scales with score (`creditLimitUsd`). The Borrow UI reads `minScore` live from the contract.
6. **Liquidity** — ERC-4626 `deposit` + ERC-7540 async redeem (`requestRedeem` → `fulfillRedeem` → `redeem`), plus a `KreditoInsurancePool` COVER reserve. **Live on Sepolia** — LPs supply USDC via the app's Liquidity step (the vault is currently unseeded, so Borrow disburses once liquidity exists).
...
| **KreditoVault** | `0xd09ecaa42eeb68c5a638d7556c41d62c38dbe5cc` | EIP-712 attestation-gated ERC-4626 + ERC-7540 **installment** lender. `issuer = 0x4b24…6978`, `asset = Circle Sepolia USDC 0x1c7D…7238`, `minScore = 600`. **Deployed; unseeded** — LPs supply USDC via the app's Liquidity step. Borrow gate (≥600) is stricter than the identity-mint threshold (≥400). Wire via `NEXT_PUBLIC_KREDITO_VAULT`. |
| **KreditoInsurancePool** | `0xfaf6200ad67d3ac5e12807a23e20dfa96adca3c8` | ERC-4626 COVER reserve paid on default, fed the protocol fee; wired to the vault (`setInsurancePool`/`setVault`). Wire via `NEXT_PUBLIC_KREDITO_INSURANCE`. |
