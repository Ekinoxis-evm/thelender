...
import { nextStep } from "~~/kredito/flow";
...
export const NextStepCard = ({ currentKey }: { currentKey: string }) => {
  ...
  return (
    <Link
      href={next.href}
      className="k-card p-5 flex items-center justify-between gap-4 hover:border-primary/40 transition-colors group"
    >
      <div>
        <p className="k-eyebrow mb-1">Next · step {next.step.toString().padStart(2, "0")}</p>
