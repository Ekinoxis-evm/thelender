...
-- Stored here so prompts/model can be reviewed and updated from the admin without a redeploy
-- (e.g. a monthly model review). The score pipeline reads the active row, falling back to the
-- in-code defaults in services/lendsignal/prompt.ts. Append-only history: a new active row
-- deactivates the previous one, so past configs stay reviewable.
